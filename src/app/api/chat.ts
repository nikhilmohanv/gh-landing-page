...
if (!process.env.OPENAI_API_KEY && !process.env.NEXT_PUBLIC_OPENAI_API_KEY) {
  throw new Error('Missing OpenAI API key. Please set the OPENAI_API_KEY environment variable in your .env.local file.');
}
...
export async function* getChatResponseStream(message: string, conversationHistory: Array<{role: string, content: string}>) {
  try {
    const messages = [
...
// Fallback non-streaming function for error cases
export async function getChatResponse(message: string, conversationHistory: Array<{role: string, content: string}>) {
  try {
    const messages = [
      { role: 'system', content: SYSTEM_PROMPT },
    ...
    return completion.choices[0]?.message?.content || 'I apologize, but I encountered an issue. Please try asking your question again.';
  } catch (error) {
    console.error('OpenAI API error:', error);
    throw error;
  }
